...
describe('rgb', () => {
  it('should return a rgb string in the correct format', () => {
    ...
    expect(randRgb()).toEqual(expect.stringMatching(expected));
  });
  ...
  it('should return a rgba string in the correct format', () => {
    ...
    expect(randRgb({ alpha: true })).toEqual(expect.stringMatching(expected));
  });

  it('should return red value between 0 and 255', () => {
    const colourValues = randRgb().match(/\d+/g)?.map(Number);
  ...
  it('should return alpha value between 0.1 and 1', () => {
    const colourValues = randRgb({ alpha: true })
      .match(/\d*(?:\.\d+)/)
      ?.map(Number);
