  ...
  describe('length is passed', () => {
    ...
    describe('length is 3', () => {
      ...
      it('should return an array length of 3, each with string legnth between min and max', () => {
        ...
        expect(result[0].length).toBeGreaterThanOrEqual(min);
        expect(result[0].length).toBeLessThanOrEqual(max);
        expect(result[1].length).toBeGreaterThanOrEqual(min);
        expect(result[1].length).toBeLessThanOrEqual(max);
        expect(result[2].length).toBeGreaterThanOrEqual(min);
        expect(result[2].length).toBeLessThanOrEqual(max);
      });
    });
  });
});
