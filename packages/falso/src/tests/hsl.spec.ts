...
describe('hsl', () => {
    it('should return a hsl string in the correct format', () => {
        ...
        expect(hsl()).toEqual(expect.stringMatching(expected));
    });
    ...
    it('should return a hsla string in the correct format', () => {
        ...
        expect(hsl({ alpha: true })).toEqual(expect.stringMatching(expected));
    });

    it('should return hue value between 0 and 359', () => {
        const colourValues = hsl().match(/\d+/g)?.map(Number);
    ...
    it('should return alpha value between 0.1 and 1', () => {
        const colourValues = hsl({ alpha: true })
            .match(/\d*(?:\.\d+)/)
            ?.map(Number);
